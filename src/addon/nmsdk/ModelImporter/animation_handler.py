...
from ..ModelExporter.utils import transform_to_matrix
...
def handle_stillframe_data(scene, anim_data: TkAnimMetadata):
    # Loop over the nodes, and then based on their names, apply the still frame transforms.
    for node in anim_data.NodeData:
        name = node.Node
        if (obj := scene.objects.get(name)) is None:
            continue
        location = anim_data.StillFrameData.Translations[node.TransIndex]
        rotation = anim_data.StillFrameData.Rotations[node.RotIndex]
        scale = anim_data.StillFrameData.Scales[node.ScaleIndex]
        matrix = transform_to_matrix(location, rotation, scale)
        obj.matrix_local = matrix


def add_animation_to_scene(scene, anim_name: str, anim_data: TkAnimMetadata, stillframe_only: bool = False):
    ...
    # StillFrameData
    if stillframe_only:
        handle_stillframe_data(scene, anim_data)
        return
    node_data_map = dict()
    rot_anim_len = len(anim_data.AnimFrameData[0].Rotations)
    trans_anim_len = len(anim_data.AnimFrameData[0].Translations)
    scale_anim_len = len(anim_data.AnimFrameData[0].Scales)
    ...
    for name, data in node_data_map.items():
        ...
        fcurves = _create_anim_channels(obj, action_name)
        _apply_animdata_to_fcurves(fcurves, data, anim_data, False)

    # If we have a mesh with joint bindings, also animate the armature
    if scene.nmsdk_anim_data.has_bound_mesh:
        ...
        for name, node_data in node_data_map.items():
            ...
            if name not in scene.nmsdk_anim_data.joints:
                continue
            ...
            for i, frame in enumerate(anim_data['AnimFrameData']):
                ...
                for key, value in animated_data.items():
                    ...
                    if key == 'Translation':
                        ...
                        # move the w value to the start to initialize the
                        # quaternion
                        rotation = Quaternion([data[3], data[0], data[1], data[2]])
                    elif key == 'Scale':
                        scale = Vector(data[:3])

                bind_data = scene.objects[name]['bind_data']
                delta_loc = location - Vector(bind_data[0].to_list())
                delta_rot = rotation.rotation_difference(Quaternion(bind_data[1].to_list()))
                ...
                for key in ['Translation', 'Rotation', 'Scale']:
                    if key in still_data:
                        if i == 0 or i == num_frames - 1:
                            _apply_pose_data(bone, DATA_PATH_MAP[key], i, action_name)
                    elif key in animated_data:
                        _apply_pose_data(bone, DATA_PATH_MAP[key], i, action_name)
    scene.nmsdk_anim_data.loaded_anims.append(anim_name)


def _apply_animdata_to_fcurves(fcurves, mapping: dict, anim_data: TkAnimMetadata,
    ...
    if use_null_transform:
        _apply_stillframe_data(loc.x, 0, num_frames)
        ...
        _apply_stillframe_data(rot.w, 1, num_frames)
        _apply_stillframe_data(sca.x, 1, num_frames)
        _apply_stillframe_data(sca.y, 1, num_frames)
        _apply_stillframe_data(sca.z, 1, num_frames)
        return
    ...
    for key, value in still_data.items():
        ...
        if key == 'Translation':
            _apply_stillframe_data(loc.x, data[0], num_frames)
            _apply_stillframe_data(loc.y, data[1], num_frames)
            _apply_stillframe_data(loc.z, data[2], num_frames)
        elif key == 'Rotation':
            _apply_stillframe_data(rot.x, data[0], num_frames)
            _apply_stillframe_data(rot.y, data[1], num_frames)
            _apply_stillframe_data(rot.z, data[2], num_frames)
            _apply_stillframe_data(rot.w, data[3], num_frames)
        elif key == 'Scale':
            _apply_stillframe_data(sca.x, data[0], num_frames)
            _apply_stillframe_data(sca.y, data[1], num_frames)
            _apply_stillframe_data(sca.z, data[2], num_frames)
    ...
    for key, value in animated_data.items():
        ...
        for i, frame in enumerate(anim_data.AnimFrameData):
            ...
            if key == 'Translation':
                _apply_animframe_data(loc.x, data[0], i)
                _apply_animframe_data(loc.y, data[1], i)
                _apply_animframe_data(loc.z, data[2], i)
            elif key == 'Rotation':
                _apply_animframe_data(rot.x, data[0], i)
                _apply_animframe_data(rot.y, data[1], i)
                _apply_animframe_data(rot.z, data[2], i)
                _apply_animframe_data(rot.w, data[3], i)
            elif key == 'Scale':
                _apply_animframe_data(sca.x, data[0], i)
                _apply_animframe_data(sca.y, data[1], i)
                _apply_animframe_data(sca.z, data[2], i)


def _apply_animframe_data(fcurve, data, frame):
    fcurve.keyframe_points[int(frame)].co = float(frame), float(data)


def _apply_stillframe_data(fcurve, data, num_frame):
...
def _apply_pose_data(bone, _type, frame, name):
    bone.keyframe_insert(data_path=_type, frame=frame, group=name)


def _create_anim_channels(obj, anim_name: str):
